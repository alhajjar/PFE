 ...
 *  \param[in] theta de temperature °C et stade viellissement de la bibitte de 0 a 100
 ...
 */
Matrice coeff_depot1(float p, Matrice eta){
    float a1 = 1.004855;
    float a2 = 1.652655;
    float a = -0.267819;
    float k = 6.214583;
    ...
    for(int i=0;i<TAILLE_LIGNE;i++){
            for(int j=0;j<TAILLE_COLONNE;j++){
              mat.setValue(i, j,log(a1+a*(-(p*p))*atan(k*eta.getValue(i,j))+a2*p));
            }
    }
    return mat;
}
 ...
 */
Matrice coeff_depot2(float p, Matrice eta){
    float a1 = -0.128715;
    float a2 = 24.589110;
    float a3 = 0.946651;
    float a4 = 0.079787;
    ...
    for(int i=0;i<TAILLE_LIGNE;i++){
            for(int j=0;j<TAILLE_COLONNE;j++){
              num = (a4*exp(p)+a5*log(1+p));
              den = 1-a1*atan(k*eta.getValue(i,j))+exp(a2*(p-a3)); 
              mat.setValue(i, j, num/den);
            }
    }
    return mat;  
 ...
 */
Matrice coeff_depot3(float p, Matrice eta){
    float a1 = 0.23001;
    float a2 = 0.86894;
    float a3 = 1.08105;
    float k = 8.22311;
    ...
    for(int i=0;i<TAILLE_LIGNE;i++){
            for(int j=0;j<TAILLE_COLONNE;j++){
                    mat.setValue(i, j, log(a2+a1*(p-(p*p))*atan(k*eta.getValue(i,j)))+a3*p);
            }
    }
    return  mat;
}
 ...
 */
Matrice coeff_depot4(float p, Matrice eta){
    float a1=0.318941;
    float a2=0.983375;
    float a3=1.669692;
    float k =-6.572036;
 ...
 */
Matrice coeff_depot5(float p, Matrice eta){
    float a1=-0.12952;
    float a2 = 0.73519;
    float a3 =1.92703;
    float k =38.70085;
    ...
    for(int i=0;i<TAILLE_LIGNE;i++){
            for(int j=0;j<TAILLE_COLONNE;j++){
                    if (p<0.5)
                       mat.setValue(i, j,log(a2+a1*(p-(p*p)*atan(-k*eta.getValue(i,j))+a3*p)));
                    else mat.setValue(i, j,log(a2+a1*(p-(p*p)*atan(k*eta.getValue(i,j))+a3*p)));
            }
    }
    return  mat;
